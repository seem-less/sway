///! Verification of each of the various IR instructions.  Mostly these checks are just to ensure
///! that they are well-formed, usually to be sure everything is properly typed.
...
pub(super) struct InstructionVerifier<'a> {
    context: &'a Context,
...
impl<'a> InstructionVerifier<'a> {
    ...
    pub(super) fn verify_instructions(&self) -> Result<(), IrError> {
